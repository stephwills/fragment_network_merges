...
def main():
    ...
    fragmentNetworkSearcher = getFragmentNetworkSearcher()
    for smiles_pair, name_pair in zip(
        smiles_pairs, name_pairs
    ):  # TODO: We may perform several parallel queries, need to talk to Tim
        fragmentNetworkSearcher.get_expansions(
            smiles_pair, name_pair, args.target, args.output_dir
        )
