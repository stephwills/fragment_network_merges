...
from rdkit.Chem import AllChem, Mol, rdForceFieldHelpers
...
class EmbeddingFilterSS(Filter_generic):
    def __init__(
        self,
        smis=None,
        synthons=None,
    ...
    @staticmethod
    def check_overlap(molA: Mol, molB: Mol, atom_clash_dist: float = config_filter.ATOM_CLASH_DIST) -> Tuple[Mol, Mol]:
        """
    ...
    def embedding(
        self, fragA: Mol, fragB: Mol, merge_mol: Mol, atom_clash_dist: float = config_filter.ATOM_CLASH_DIST
    ) -> list:
        """
    ...
    def filter_smi(
        ...
        embedded_mols = self.embedding(fragA, fragB, merge, atom_clash_dist)
    ...
    def filter_all(
        ...
        res = Parallel(n_jobs=cpus, backend="multiprocessing")(
            delayed(self.filter_smi)(smi, self._fragmentA, self._fragmentB, **kwargs)
